...
async def listen(c, m):
    ...
    if v["codigoRetorno"] == 98:
        await m.reply_text("⚠️ <b>{}.</b>".format(v["mensagemRetorno"]), quote=True, parse_mode = "HTML")
			
    else:
        await m.reply_text(template.format(l.now().strftime("%d/%m/%Y ás %H:%M:%S"), s, 
	            			 	         v["chassi"],
	            		     	         v["modelo"], 
	            			 			 v["cor"].upper(), 
	            			 		     v["ano"], 
	            			 			 v["municipio"].upper(), 
	            			 			 v["uf"], 
	            			 			 v["situacao"]), quote=True, parse_mode = "HTML")