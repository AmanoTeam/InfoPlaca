...
# PADRAO CONVENCIONAL	
    ...
    if re.search(regex_c, m.query):
        ...
        with httpx.Client(proxies=PROXY) as cli:
            h = cli.get(f"http://api.masterplaca.devplank.com/v51/placa/{f}/json").json()
            ...
            await m.answer(
                [
                        InlineQueryResultArticle(
	                        ...
	                        message_text = f"⚠️ <b>{h['mensagemRetorno']}.</b>", parse_mode = "HTML"))
                ])
            ...
            await m.answer(
                [
                        InlineQueryResultArticle(
	                        ...
	                        thumb_url = "https://piics.ml/i/015.png",
	                        input_message_content = InputTextMessageContent(
	                        str(template.format(l.now().strftime("%d/%m/%Y às %H:%M:%S"), g, 
	            			                	h["chassi"],
	            				            	h["modelo"], 	            										  
	            				            	h["cor"].upper(), 
	            				            	h["ano"], 
            ...
            await m.answer(
                 [
                        InlineQueryResultArticle(
	                        ...
	                        thumb_url = "https://piics.ml/i/015.png",
	                        input_message_content = InputTextMessageContent(
	                        message_text = f"⚠️ <b>{q['mensagemRetorno']}.</b>", parse_mode = "HTML"))
                 ])
            ...
            await m.answer(
                 [
                        InlineQueryResultArticle(
	                        ...
	                        thumb_url = "https://piics.ml/i/015.png",
	                        input_message_content = InputTextMessageContent(
	                        str(template.format(l.now().strftime("%d/%m/%Y às %H:%M:%S"), p.upper(), 
	            			 				   q["chassi"],
	            		     				   q["modelo"], 
	            			 				   q["cor"].upper(), 
	            			 				   q["ano"], 
